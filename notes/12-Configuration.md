...
```viml
    ...
    set expandtab
    set shiftwidth=4
    set softtabstop=4  " At the time I didn't know about this one
```

1. Using `syntax off` turned off the "ugly colors" that I was not used to.
...
The existence of either `~/.vimrc` or `~/.vim/vimrc` caused the `defaults.vim`
file not to be run.  This was extremely confusing to me.  Many useful Vim
behaviors disappeared and I had no idea how to get them back.  Without even
knowing of the existence of `defaults.vim`, much less its location, I was lost.

After much experimenting, this is what I came up with after about a
year for a `~/.vim/vimrc` file.

```viml
    """ Preliminaries

    "" Always do first, should be the default
    set nocompatible
...
Then I discovered Neovim.  The following is my first `~/.config/nvim/init.lua`
file.  Basically a direct translation of a later version of the above.
