...
```console
    $ nvim -p[n]    # Open n tab pages (default: one for each file)
    $ nvim -o[n]    # Open n windows (default: one for each file)
    $ nvim -O[n]    # Like -o but split vertically
```
...
| `:sh`        | replace editing session with new shell (not Neovim) |
...
| Command | Description                                                |
|:------- |:---------------------------------------------------------- |
| `:term`               | opens default shell in the current window    |
