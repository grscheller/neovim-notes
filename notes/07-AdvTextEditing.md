...
| `*`       | jump to next word cursor on/before, takes count   |
| `#`       | jump to prev word cursor on/before, takes count   |
...
| Command      | Description                                        |
|:------------:|:-------------------------------------------------- |
| `x`          | delete char under cursor, put in default register  |
| `X`          | delete char before cursor, put in default register |
...
| `d0`    | delete everything before cursor & put in default register  |
...
| `i<` | select what is between `<` and `>`                         |
| `a<` | same as above and the `<` and `>` too                      |
...
| `<C-c>`           | Quit *insert mode*, InsertLeave not triggered |
...
| prev: [Vim Specific Features][6] | [Home][0] | next: [Mult Windows][8] |
